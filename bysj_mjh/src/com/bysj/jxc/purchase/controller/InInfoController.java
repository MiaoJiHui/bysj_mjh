...
import com.bysj.jxc.purchase.service.InInfoService;
...
@Controller
@RequestMapping("in")
public class InInfoController extends BaseController{
	...
	public ModelAndView list()throws Exception{
		mv.clear();
		pd = this.getPageData();
		List<InInfo> list = inInfoService.listPd(pd);
		mv.setViewName("dwz/in_storage");
	...
	//	inInfoService;.insert(pd);
		if(!inInfoService.insert(pd)){
		     String json = "{\"statusCode\":300,\"message\":\"程序出错了！\"}";
		}
		String json = "{\"statusCode\":200,\"message\":\"操作成功了！\",\"callbackType\":\"closeCurrent\",\"navTabId\":\"in_information\"}";
	...
	public void delete(HttpServletResponse response)throws Exception{
		pd = this.getPageData();
		if(!inInfoService.delete(pd)){
			 String json = "{\"statusCode\":300,\"message\":\"程序出错了！\"}";
		}
		String json = "{\"statusCode\":200,\"message\":\"操作成功了！\",\"navTabId\":\"in_information\"}";
		response.getWriter().write(json);
		response.getWriter().flush();
		response.getWriter().close();
	}
	...
	public ModelAndView update() throws Exception {
		mv.clear();
		pd = this.getPageData();
		pd = inInfoService.listById(pd);
		Gson gson = new Gson();
		mv.setViewName("dwz/in_modify");
		// mv.addObject("list",list);
		mv.addObject("pd", pd);
		return mv;
	}
	...
	public void saveUpdate(HttpServletResponse response) throws Exception {
		pd = this.getPageData();
		if(!inInfoService.update(pd))
		{
			String json = "{\"statusCode\":300,\"message\":\"程序出错了！\"}";
		}
		String json = "{\"statusCode\":200,\"message\":\"修改数据成功！\",\"callbackType\":\"closeCurrent\",\"navTabId\":\"in_information\"}";
		response.getWriter().write(json);
		response.getWriter().flush();
		response.getWriter().close();
	}
	...
	public void excelExport(HttpServletResponse response) throws Exception {
		...
		for (int i = 0; i < list.size(); i++) { // 行数
			...
			rowDataModel.getData().add(list.get(i).getStorage_address());
		...
		model.setPath("F:/excel/in_info.xls"); //   这样只是把excel上传到服务器端，不是客户端即本机。所以要用下面这个方法来实现另存为到本地。
