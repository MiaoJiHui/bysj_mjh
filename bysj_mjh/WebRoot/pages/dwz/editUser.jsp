...
<div class="pageContent">
	<form method="post" action="header/saveUpdate?2=2"
			...
			<table class="formTable" style="">
				<tbody>
					<tr>
						...
						<input type="text" name="username" class="required" value="${pd.username}" alt="请输入用户名 "/> 
					...
					<tr>
						...
						<td style="width:150px;"><input name="password" id="cp_newPassword"
							class="required alphanumeric"  type="password" size="" value="${pd.password }"
					...
					<tr>
						...
						<td style="width:150px;"><input name="password2" id="pwd2"
							equalTo="#cp_newPassword" class="required alphanumeric" type="password" size="" value="${pd.password }"
							alt="请输入密码" /> <span class="inputInfo">必填项</span>
						</td>
					</tr>
					...
					<tr>
						...
						<select name="limitUser" id="limitUser" >
								   <option value="${pd.limitUser }">${pd.limitUser }</option>
								   <c:if test="${pd.limitUser eq 'guest' }">
								      <option value="admin">admin</option>
								   </c:if>
								   <c:if test="${pd.limitUser eq 'admin' }">
