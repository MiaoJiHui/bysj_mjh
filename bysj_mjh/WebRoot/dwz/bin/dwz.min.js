var DWZ = {
    ...
    ajaxError: function (xhr, ajaxOptions, thrownError) {
        if (alertMsg) {
            ...
            alert("Http status2222: " + xhr.status + " " + xhr.statusText + "\najaxOptions: " + ajaxOptions + "\nthrownError:" + thrownError + "\n" + xhr.responseText);
        }
    },
    ...
    init: function (pageFrag, options) {
        ...
        jQuery.ajax({
            ...
            cache: false,
            error: function (xhr) {
                alert('Error loading XML document: ' + pageFrag + "\nHttp status3333: " + xhr.status + " " + xhr.statusText);
            },
    ...
    $.fn.extend({
        ajaxUrl: function (op) {
            ...
            $.ajax({
                ...
                url: op.url,
                data: op.data,
                global:op.glable==false?false:true,
                cache: false,
        ...
        loadUrl: function (url, data, callback,glable) {
            $(this).ajaxUrl({url: url, data: data,glable:glable, callback: callback});
        },
        initUI: function () {
...
//----------YaoJ：带分页、且使用.table表格的列表页面，自动判断表格高度
                var ptype = navTab.getCurrentPanel() != null ? navTab.getCurrentPanel() : $.pdialog.getCurrent();
                if (iLayoutH == 0 && $("div.pagination", ptype).size() > 0) {
                    var BarH = 0;
                    $(".panelBar", ptype).each(function () {
                        BarH += $(this).height() + 2;
                    });
                    iLayoutH = $(".pageHeader", ptype).height() + BarH + $(".gridHeader", ptype).height() + 11;
                }
//----------END
                var iH = iRefH - iLayoutH > 50 ? iRefH - iLayoutH : 50;
                if ($this.isTag("table")) {
                    $this.attr("layoutH",iLayoutH+25);
...
(function ($) {
    $.extend($.fn, {
        ...
        genTree: function (options) {
            ...
            return this.each(function () {
                ...
                if (tree.size() > 0) {
                    ...
                    var i = node.attr('icon');
                    var c = (i == null ? "file" : i);
                    $(">div", node).prepend("<div class='node'></div>" + (op.ckbox ? "<div class='ckbox unchecked'></div>" : "") + (op.icon ? "<div class='" + c + "'></div>" : ""));
                    addSpace(op.level, node);
        ...
        _checkParent: function () {
            ...
            $(">div>.ckbox", parent).removeClass("unchecked").removeClass(rClass).addClass(aClass).children().attr("checked", "checked");
            parent._checkParent();
        }
    });
})(jQuery);
            ...
            slide: function (options, additions) {
                ...
                options.toHide.filter(":hidden").each(options.complete).end().filter(":visible").animate({height: "hide"}, {
                    ...
                    complete: function () {
                        ...
                        // ShenJH 修改菜单功能
                        options.toShow.css({overflow: "hidden"});
                        options.toShow.css({overflow: "auto"});
                        // ShenJH 添加了菜单滚动的功能
                        menuScroll($(".accordionContent:visible"));
                        options.complete();
...
(function ($) {
    $.pdialog = {
        ...
        reload: function (url, options,data) {
            var op = $.extend({data: (data?data:{}), dialogId: "", callback: null}, options);
...
function validateCallback(form, callback, confirmMsg) {
    ...
    var _submitFn = function () {
        //Yaoj 百分比转换，转换金额数字
        $form.find(".percent").each(function(){
            $(this).val($(this).val()/100);
        });
        $form.find(".ci").each(function () {
    ...
    $.fn.extend({
        ...
        itemDetail: function () {
            return this.each(function () {
                ...
                $table.find("tr:first th[type]").each(function (i) {
                    ...
                    var field = {
                        ...
                        defaultText: $th.attr("defaultText") || "",
            ...
            function initSuffix($tbody) {
                $tbody.find('>tr').not(".exist").each(function (i) {
            ...
            function tdHtml(field) {
                ...
                switch (field.type) {
                    case 'readonly':
                        html = (field.defaultText?'<span>'+field.defaultText+'</span>':'')+'<input type="text"  readonly name="' + field.name + '" value="' + field.defaultVal + '" size="' + field.size + '" class="' + field.fieldClass + '" ' + attrFrag + '/>';
                        break;
                    case 'hidden':
                        html = '<span>'+field.defaultText+'</span><input type="hidden" name="' + field.name + '" value="' + field.defaultVal + '" size="' + field.size + '" class="' + field.fieldClass + '" ' + attrFrag + '/>';
                        break;
                    case 'radio':
                        html = "<input type='radio' name='"+field.name+"' value='1' class='"+field.fieldClass+"' class='"+field.fieldClass+"' onclick='ck(this)'/>";
                        break;
                    case 'del':
                    ...
                    case 'date':
                        ...
                        break;
                    case 'textarea':
                        html = '<textarea name="' + field.name + '" class="' + field.fieldClass + '" style="width:95%" rows="'+field.rows+'" ></textarea>';
                        break;
                    default:
                        html = (field.defaultText?'<span>'+field.defaultText+'</span>':'')+'<input type="text" name="' + field.name + '" value="' + field.defaultVal + '" size="' + field.size + '" class="' + field.fieldClass + '" ' + attrFrag + '/>';
                        break;
                }
